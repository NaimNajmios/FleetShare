...
public interface VehiclePriceHistoryRepository extends JpaRepository<VehiclePriceHistory, Long> {
    ...
    @Query(value = "SELECT t.price_id, t.vehicle_id, t.rate_per_day, t.effective_start_date " +
            "FROM ( " +
            "  SELECT price_id, vehicle_id, rate_per_day, effective_start_date, " +
            "         ROW_NUMBER() OVER (PARTITION BY vehicle_id ORDER BY effective_start_date DESC, price_id DESC) as rn " +
    ...
    List<VehiclePriceHistory> findLatestPricesForVehiclesNative(@Param("vehicleIds") java.util.Collection<Long> vehicleIds);
}
